class Ticket:
    ...
    def calculate_price(self):  # Method to calculate the price of the ticket
        ...
        # 50% discount for groups
            ...
            if self.ticket_type == "SpecialEvent":
                return (self.__event_details.price * 0.5 * self.__group) * (1 + vat_rate)  # Calculate price with discount for special events for the group
            else:
                return (base_price * 0.5 * self.__group) * (1 + vat_rate)  # Calculate price with discount for regular events for the group

        # Individual ticket prices for special events
        elif self.__ticket_type == "SpecialEvent":
             return self.__event_details.price * (1 + vat_rate)   # Calculate price without discount for special events
    ...
    def __str__(self):  # Method to represent Ticket object as a string
        return f"Ticket: {self.__ticket_type}, Event: {self.__event_details}, Visitor: {self.__visitor.name}, Price: {self.__price}"    # Formatting string representation
